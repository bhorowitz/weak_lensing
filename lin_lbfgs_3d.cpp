...
public:
    ...
    {
        for(auto it = c_.begin(); it != c_.end(); ++it)
            *it = std::complex<double>(((double) rand() / (RAND_MAX))*100., ((double) rand() / (RAND_MAX))*100.);

        for(auto it = q_.begin(); it != q_.end(); ++it)
            *it = ((double) rand() / (RAND_MAX))*100.;
    }
...
int main(int argc, char *argv[])
{
	...
	bool weakLensingMapOnly = parser.getBool("weak_lensing_map_only", false);
        //const std::string loc = parser.getStr("file_prefix", "");
        const std::string mask_file = parser.getStr("mask_file", "mask.txt");

	if(weakLensing and weakLensingMapOnly){
	  output_screen("NOTE! both weakLensing and weakLensingMapOnly enabled, disabling map only!" << std::endl);
	  weakLensingMapOnly = false;
	}
        parser.dump();
        ...
        */
	    ...
	    if(weakLensingMapOnly){
	      for(int i = 0; i < N * N; ++i)
		{
		  dataGamma1[i] += noiseGen.generate() * sigmaNoise[i] * 2;
		  dataGamma2[i] += noiseGen.generate() * sigmaNoise[i] * 2;
		}}
	      else{
		for(int i = 0; i < N * N; ++i)
		  {
